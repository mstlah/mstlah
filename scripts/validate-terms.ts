...
function validateTermStructure(
  term: ParsedTerm,
  content: string,
  filename: string,
): ValidationError[] {
  ...
  if (!valid) {
    ...
    if (!Array.isArray(term.tags)) {
      errors.push({
        ...
        type: "required",
        message:
          "Tags section missing or invalid (expected: ## Tags with list items)",
    ...
    if (!Array.isArray(term.arabicWords)) {
      errors.push({
        ...
        type: "required",
        message:
          "Arabic Words section missing or invalid (expected: # Arabic Words with ## Arabic Word subsections)",
  ...
  if (!term.description || term.description.trim().length === 0) {
    errors.push({
      ...
      type: "required",
      message:
        "Missing or empty description (expected: ## Description with Arabic content)",
  ...
  if (term.tags.length === 0) {
    errors.push({
      ...
      type: "required",
      message:
        "At least one tag is required (expected: - tagname in ## Tags section)",
      line: findLineNumber(lines, /^## Tags$/),
    });
  } else {
    const nonArabicTags = term.tags.filter(
      (tag: string) => !containsArabic(tag),
    );
    if (nonArabicTags.length > 0) {
      errors.push({
        file: filename,
        type: "content",
        message: `All tags must be in Arabic. Non-Arabic tags: ${
          nonArabicTags.join(", ")
        }`,
        line: findLineNumber(lines, /^## Tags$/),
      });
  ...
  if (term.arabicWords.length === 0) {
    errors.push({
      ...
      type: "required",
      message:
        "At least one Arabic word is required (expected: ## Arabic Word in # Arabic Words section)",
      line: findLineNumber(lines, /^# Arabic Words$/),
    });
  } else {
    term.arabicWords.forEach((aw: ArabicWord, index: number) => {
      if (!aw.word || aw.word.trim().length === 0) {
        ...
        errors.push({
          ...
          line: findLineNumber(
            lines,
            new RegExp(`^## ${escapeRegex(aw.word)}$`),
          ),
        });
      }
      ...
      if (!aw.approvedBy || aw.approvedBy.length === 0) {
        errors.push({
          ...
          type: "required",
          message:
            `Arabic word "${aw.word}" must have at least one approver (expected: ### Approved By with - username)`,
  ...
  results.forEach((result: ValidationResult) => {
    if (result.valid) {
      ...
      if (result.data) {
        ...
        lines.push(
          `  Tags: ${result.data.tagsCount}, Arabic Words: ${result.data.arabicWordsCount}`,
        );
      }
    } else {
      invalidCount++;
      ...
      result.errors.forEach((error: ValidationError) => {
        ...
        lines.push(
          `  [${error.type.toUpperCase()}] ${error.message}${lineInfo}`,
        );
      });
    }
    lines.push("");
